...
local function updatePreviewPosition()
    ...
    Citizen.CreateThread(function()
        while previewProp and holdingProp do
            local playerPed = PlayerPedId()
...
local function showPreview()
    ...
    SetEntityCompletelyDisableCollision(previewProp, true, true)
...
local function pickupProp()
    ...
    if object and DoesEntityExist(object) then
        ...
        AttachEntityToEntity(holdingProp, playerPed, GetPedBoneIndex(playerPed, 57005), 0.1, -0.15, -0.05, 0.0, 0.0, 0.0, true, true, false, true, 1, true)
        disableActions()
        showPreview()
    end
end
...
local function placeProp()
    if holdingProp then
        ...
        FreezeEntityPosition(placedProp, false)
